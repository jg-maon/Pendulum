...
public:
	...
	*/
	CRoboticArm(const mymath::Vec3f& pos, int dir = static_cast<int>(ArmDirectin::RIGHT_UP));
	/*
	@brief	座標指定したオブジェクト生成
	@param	[in]	x	X座標
	@param	[in]	y	Y座標
	...
	*/
	CRoboticArm(float x, float y, int dir = static_cast<int>(ArmDirectin::RIGHT_UP), float z = 0.5f);
	~CRoboticArm();
	/*
	@brief	更新処理
	@return	なし
